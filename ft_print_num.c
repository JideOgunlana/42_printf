...
/*   Updated: 2022/05/21 17:34:35 by bogunlan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_num_flags(t_printf *val, int num, char *str)
{
	...
	else if (val->spc && num >= 0 && val->width < ft_strlen(str))
	{
		ft_putchar_fd(' ', 1);
		val->tl += 1;
		val->spc = FALSE;
...
void	ft_print_num(t_printf *val)
{
	...
	check_num_flags(val, num, str);
	if (val->width > ft_strlen(str))
		val->tl += width_count;
	if (val->point > ft_strlen(str) -1)
	{
